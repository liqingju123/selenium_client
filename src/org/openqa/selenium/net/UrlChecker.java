...
public class UrlChecker {
	...
	private static final ExecutorService THREAD_POOL = Executors.newCachedThreadPool(new ThreadFactory() {
		public Thread newThread(Runnable r) {
			Thread t = new Thread(r, "UrlChecker-" + THREAD_COUNTER.incrementAndGet()); // Thread
																						// safety
																						// reviewed
			t.setDaemon(true);
			return t;
	...
	public void waitUntilAvailable(long timeout, TimeUnit unit, final URL... urls) throws TimeoutException {
		...
		try {
			...
			throw new TimeoutException(String.format("Timed out waiting for %s to be available after %d ms",
	...
	public void waitUntilUnavailable(long timeout, TimeUnit unit, final URL url) throws TimeoutException {
		long start = System.nanoTime();
		...
		try {
			...
			throw new TimeoutException(String.format("Timed out waiting for %s to become unavailable after %d ms", url,
	...
	private HttpURLConnection connectToUrl(URL url) throws IOException {
		System.out.println("UrlChecker=== " + url.getPath());
