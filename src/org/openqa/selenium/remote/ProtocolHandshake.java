...
public class ProtocolHandshake {
	...
	/**
	 * Capability names that should never be sent across the wire to a w3c
	 * compliant remote end.
	 */
	private final Set<String> UNUSED_W3C_NAMES = ImmutableSet.<String> builder().add("firefox_binary")
			.add("firefox_profile").add("marionette").build();
	...
	public Result createSession(HttpClient client, Command command) throws IOException {
		...
		// We don't know how large the generated JSON is going to be. Spool it
		// to disk, and then read
		// the file size, then stream it to the remote end. If we could be sure
		// the remote end could
		// cope with chunked requests we'd use those. I don't think we can.
		// *sigh*
		Path jsonFile = Files.createTempFile("new-session", ".json");

		try (BufferedWriter fileWriter = Files.newBufferedWriter(jsonFile, UTF_8);
	...
	private void streamJsonWireProtocolParameters(JsonWriter out, Gson gson, JsonObject des, JsonObject req)
			throws IOException {
	...
	private void streamW3CProtocolParameters(JsonWriter out, Gson gson, JsonObject des, JsonObject req)
			throws IOException {
		// Technically we should be building up a combination of "alwaysMatch"
		// and "firstMatch" options.
		// We're going to do a little processing to figure out what we might be
		// able to do, and assume
		// that people don't really understand the difference between required
		// and desired (which is
		// commonly the case). Wish us luck. Looking at the current
		// implementations, people may have
		// set options for multiple browsers, in which case a compliant W3C
		// remote end won't start
		// a session. If we find this, then we create multiple firstMatch
		// capabilities. Furrfu.
		...
		// Edge: none given
		// IEDriver: ignoreZoomSetting, initialBrowserUrl,
		// enableElementCacheCleanup,
		// browserAttachTimeout, enablePersistentHover, requireWindowFocus,
		// logFile, logLevel, host,
		// extractPath, silent, ie.*
		// Opera: operaOptions
		// SafariDriver: safari.options
		//
		// We can't use the constants defined in the classes because it would
		// introduce circular
		// dependencies between the remote library and the implementations. Yay!

		Map<String, ?> chrome = Stream.of(des, req).map(JsonObject::entrySet).flatMap(Collection::stream)
				.filter(entry -> ("browserName".equals(entry.getKey()) && CHROME.equals(entry.getValue().getAsString()))
						|| "chromeOptions".equals(entry.getKey()))
				.distinct().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (left, right) -> right));

		Map<String, ?> edge = Stream.of(des, req).map(JsonObject::entrySet).flatMap(Collection::stream)
				.filter(entry -> ("browserName".equals(entry.getKey()) && EDGE.equals(entry.getValue().getAsString())))
				.distinct().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (left, right) -> right));

		Map<String, ?> firefox = Stream.of(des, req).map(JsonObject::entrySet).flatMap(Collection::stream)
				.filter(entry -> ("browserName".equals(entry.getKey())
						&& FIREFOX.equals(entry.getValue().getAsString())) || entry.getKey().startsWith("firefox_")
						|| entry.getKey().startsWith("moz:"))
				.distinct().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (left, right) -> right));

		Map<String, ?> ie = Stream.of(req, des).map(JsonObject::entrySet).flatMap(Collection::stream)
				.filter(entry -> ("browserName".equals(entry.getKey()) && IE.equals(entry.getValue().getAsString()))
						|| "browserAttachTimeout".equals(entry.getKey())
						|| "enableElementCacheCleanup".equals(entry.getKey())
						|| "enablePersistentHover".equals(entry.getKey()) || "extractPath".equals(entry.getKey())
						|| "host".equals(entry.getKey()) || "ignoreZoomSetting".equals(entry.getKey())
						|| "initialBrowserZoom".equals(entry.getKey()) || "logFile".equals(entry.getKey())
						|| "logLevel".equals(entry.getKey()) || "requireWindowFocus".equals(entry.getKey())
						|| "silent".equals(entry.getKey()) || entry.getKey().startsWith("ie."))
				.distinct().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (left, right) -> right));

		Map<String, ?> opera = Stream.of(des, req).map(JsonObject::entrySet).flatMap(Collection::stream)
				.filter(entry -> ("browserName".equals(entry.getKey())
						&& OPERA_BLINK.equals(entry.getValue().getAsString()))
						|| ("browserName".equals(entry.getKey()) && OPERA.equals(entry.getValue().getAsString()))
						|| "operaOptions".equals(entry.getKey()))
				.distinct().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (left, right) -> right));

		Map<String, ?> safari = Stream.of(des, req).map(JsonObject::entrySet).flatMap(Collection::stream)
				.filter(entry -> ("browserName".equals(entry.getKey()) && SAFARI.equals(entry.getValue().getAsString()))
						|| "safari.options".equals(entry.getKey()))
				.distinct().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue, (left, right) -> right));

		Set<String> excludedKeys = Stream.of(chrome, edge, firefox, ie, opera, safari).map(Map::keySet)
				.flatMap(Collection::stream).distinct().collect(ImmutableSet.toImmutableSet());

		JsonObject alwaysMatch = Stream.of(des, req).map(JsonObject::entrySet).flatMap(Collection::stream)
				.filter(entry -> !excludedKeys.contains(entry.getKey())).filter(entry -> entry.getValue() != null)
				.filter(entry -> !UNUSED_W3C_NAMES.contains(entry.getKey())) // We
																				// never
																				// want
																				// to
																				// send
																				// this
				.distinct()
				.collect(Collector.of(JsonObject::new, (obj, e) -> obj.add(e.getKey(), e.getValue()), (left, right) -> {
		...
		// Now, hopefully we're left with just the browser-specific pieces. Skip
		// the empty ones.
		JsonArray firstMatch = Stream.of(chrome, edge, firefox, ie, opera, safari).filter(map -> !map.isEmpty())
				.map(map -> {
		...
		// TODO(simon): transform some capabilities that changed in the spec
		// (timeout's "pageLoad")
	...
	private Optional<Result> createSession(HttpClient client, InputStream newSessionBlob, long size)
		...
		// If the result was an error that we believe has to do with the remote
		// end failing to start the
		...
		// The old geckodriver prior to 0.14 returned "value" as the thing
		// containing the session id.
		// Later versions follow the (amended) w3c spec and return the
		// capabilities in a field called
		// "value". The most recent versions of the spec contain any errors
	...
	private void streamGeckoDriver013Parameters(JsonWriter out, Gson gson, JsonObject des, JsonObject req)
			throws IOException {
		out.name("desiredCapabilities");
		gson.toJson(des, out);
		out.name("requiredCapabilities");
